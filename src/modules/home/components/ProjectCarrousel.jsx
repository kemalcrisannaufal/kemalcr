...
const CardItem = (props) => {
  ...
  return (
    <a href={destination} className="block">
      <div className="card-container h-48 p-0 rounded-lg overflow-hidden border mx-2 flex items-center justify-center relative hover:bg-gray-100 dark:hover:bg-neutral-500 transition duration-300 shadow-lg">
        <img
          src={image}
          alt={title}
          className="card-image object-cover w-16 h-12 md:w-1/4 md:h-1/4"
        />
        <p className="card-title text-md line-clamp-1 absolute top-2 left-4 text-neutral-800 font-medium dark:text-white">
          {title}
        </p>
        <p className="text-[10px] sm:text-xs px-5 absolute left-0 top-[70%] tracking-wider text-neutral-600 line-clamp-2 lg:text-sm dark:text-white">
          {description}
        </p>
      </div>
    </a>
